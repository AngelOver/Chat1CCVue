...
var dotenv7 = __toESM(require("dotenv"));
...
// src/utils/textAudit.ts
init_cjs_shims();
var import_node_fetch = __toESM(require("node-fetch"));
var BaiduTextAuditService = class {
  constructor(options) {
    this.options = options;
  }
  async containsSensitiveWords(text) {
    ...
    const url2 = `https://aip.baidubce.com/rest/2.0/solution/v1/text_censor/v2/user_defined?access_token=${this.accessToken}`;
    let headers;
    const response = await (0, import_node_fetch.default)(url2, { headers, method: "POST", body: `text=${encodeURIComponent(text)}` });
  ...
  async refreshAccessToken() {
    ...
    try {
      if (this.accessToken && Math.floor((/* @__PURE__ */ new Date()).getTime() / 1e3) <= this.expiredTime)
        return true;
      const url2 = `https://aip.baidubce.com/oauth/2.0/token?client_id=${this.options.apiKey}&client_secret=${this.options.apiSecret}&grant_type=client_credentials`;
      let headers;
      const response = await (0, import_node_fetch.default)(url2, { headers });
      const data = await response.json();
      this.accessToken = data.access_token;
      this.expiredTime = Math.floor((/* @__PURE__ */ new Date()).getTime() / 1e3) + +data.expires_in;
      return true;
    } catch (error3) {
      global.console.error(`\u767E\u5EA6\u5BA1\u6838${error3}`);
    }
    return false;
  }
};
var textAuditServices = {
...
var UserInfo = class {
  constructor(email, password) {
    ...
    this.status = 4 /* PreVerify */;
    this.createTime = (/* @__PURE__ */ new Date()).toLocaleString();
    this.verifyTime = null;
    this.updateTime = (/* @__PURE__ */ new Date()).toLocaleString();
  }
};
var ChatRoom = class {
  constructor(userId, title, roomId) {
...
var ChatInfo = class {
  constructor(roomId, uuid, prompt, options) {
    this.status = 0 /* Normal */;
    this.roomId = roomId;
    this.uuid = uuid;
    this.prompt = prompt;
    this.options = options;
    this.dateTime = (/* @__PURE__ */ new Date()).getTime();
  }
};
var ChatUsage = class {
  constructor(userId, roomId, chatId, messageId, usage) {
    this.userId = userId;
    this.roomId = roomId;
    this.chatId = chatId;
...
var Config = class {
  constructor(_id, timeoutMs, apiKey, apiDisableDebug, accessToken, apiBaseUrl, apiModel2, reverseProxy, socksProxy, socksAuth, httpsProxy, siteConfig, mailConfig, auditConfig) {
    this._id = _id;
    this.timeoutMs = timeoutMs;
    this.apiKey = apiKey;
    this.apiDisableDebug = apiDisableDebug;
    this.accessToken = accessToken;
    this.apiBaseUrl = apiBaseUrl;
    this.apiModel = apiModel2;
    this.reverseProxy = reverseProxy;
    this.socksProxy = socksProxy;
    this.socksAuth = socksAuth;
...
var SiteConfig = class {
  constructor(siteTitle, loginEnabled, loginSalt, registerEnabled, registerReview, registerMails, siteDomain) {
    this.siteTitle = siteTitle;
    this.loginEnabled = loginEnabled;
    this.loginSalt = loginSalt;
    this.registerEnabled = registerEnabled;
    this.registerReview = registerReview;
    this.registerMails = registerMails;
    this.siteDomain = siteDomain;
  }
};
var MailConfig = class {
  constructor(smtpHost, smtpPort, smtpTsl, smtpUserName, smtpPassword) {
...
var AuditConfig = class {
  constructor(enabled, provider, options, textType, customizeEnabled, sensitiveWords) {
...
function isTextAuditServiceProvider(value) {
  return value === "baidu";
}

// src/storage/mongo.ts
init_cjs_shims();
...
var chatCol = client.db("chatgpt2").collection("chat");
var roomCol = client.db("chatgpt2").collection("chat_room");
var userCol = client.db("chatgpt2").collection("user");
var configCol = client.db("chatgpt2").collection("config");
var usageCol = client.db("chatgpt2").collection("chat_usage");
async function insertChat(uuid, text, roomId, options) {
  const chatInfo = new ChatInfo(roomId, uuid, text, options);
  await chatCol.insertOne(chatInfo);
  return chatInfo;
}
async function getChat(roomId, uuid) {
  return await chatCol.findOne({ roomId, uuid });
}
async function updateChat(chatId, response, messageId, usage, previousResponse) {
  const query = { _id: new import_mongodb.ObjectId(chatId) };
  const update = {
    $set: {
      "response": response,
...
async function deleteChatRoom(userId, roomId) {
  const result = await roomCol.updateOne({ roomId, userId }, { $set: { status: 1 /* Deleted */ } });
  await clearChat(roomId);
  return result;
}
async function updateRoomPrompt(userId, roomId, prompt) {
  const query = { userId, roomId };
  const update = {
    $set: {
      prompt
...
async function getChatRooms(userId) {
  const cursor = await roomCol.find({ userId, status: { $ne: 1 /* Deleted */ } });
  const rooms = [];
  await cursor.forEach((doc) => rooms.push(doc));
  return rooms;
}
async function getChatRoom(userId, roomId) {
  return await roomCol.findOne({ userId, roomId, status: { $ne: 1 /* Deleted */ } });
}
async function existsChatRoom(userId, roomId) {
  const room = await roomCol.findOne({ roomId, userId });
  return !!room;
}
async function deleteAllChatRooms(userId) {
  await roomCol.updateMany({ userId, status: 0 /* Normal */ }, { $set: { status: 1 /* Deleted */ } });
  await chatCol.updateMany({ userId, status: 0 /* Normal */ }, { $set: { status: 1 /* Deleted */ } });
}
async function getChats(roomId, lastId) {
  if (!lastId)
    lastId = (/* @__PURE__ */ new Date()).getTime();
  const query = { roomId, uuid: { $lt: lastId }, status: { $ne: 1 /* Deleted */ } };
...
async function deleteChat(roomId, uuid, inversion) {
  ...
  if (chat.status === 2 /* InversionDeleted */ && !inversion) {
  } else if (chat.status === 3 /* ResponseDeleted */ && inversion) {
  } else if (inversion) {
    update = {
      $set: {
        status: 2 /* InversionDeleted */
      }
    };
  } else {
    update = {
      $set: {
        status: 3 /* ResponseDeleted */
      }
    };
  }
...
async function createUser(email, password) {
  ...
  if (email === process.env.ROOT_USER)
    userInfo.status = 0 /* Normal */;
  await userCol.insertOne(userInfo);
  return userInfo;
}
async function updateUserInfo(userId, user) {
  const result = userCol.updateOne(
...
async function updateUserPassword(userId, password) {
  const result = userCol.updateOne(
    { _id: new import_mongodb.ObjectId(userId) },
    { $set: { password, updateTime: (/* @__PURE__ */ new Date()).toLocaleString() } }
  );
  return result;
}
...
async function getUserById(userId) {
  return await userCol.findOne({ _id: new import_mongodb.ObjectId(userId) });
}
async function verifyUser(email, status) {
  email = email.toLowerCase();
  return await userCol.updateOne({ email }, { $set: { status, verifyTime: (/* @__PURE__ */ new Date()).toLocaleString() } });
}
async function getConfig() {
  return await configCol.findOne();
}
async function updateConfig(config8) {
  const result = await configCol.replaceOne({ _id: config8._id }, config8, { upsert: true });
...
async function getCacheConfig() {
  ...
  cacheExpiration = now + 1 * 60 * 1e3;
  return Promise.resolve(cachedConfig);
}
async function getOriginConfig() {
  let config8 = await getConfig();
  if (config8 == null) {
    config8 = new Config(
      new import_mongodb2.ObjectId(),
      !isNaN(+process.env.TIMEOUT_MS) ? +process.env.TIMEOUT_MS : 30 * 1e3,
      ...
      process.env.SOCKS_PROXY_USERNAME && process.env.SOCKS_PROXY_PASSWORD ? `${process.env.SOCKS_PROXY_USERNAME}:${process.env.SOCKS_PROXY_PASSWORD}` : "",
    ...
    if (config8.siteConfig.loginEnabled === void 0)
      config8.siteConfig.loginEnabled = isNotEmptyString(process.env.AUTH_SECRET_KEY);
    if (config8.siteConfig.loginSalt === void 0)
      config8.siteConfig.loginSalt = process.env.AUTH_SECRET_KEY;
    if (config8.apiDisableDebug === void 0)
      config8.apiDisableDebug = process.env.OPENAI_API_DISABLE_DEBUG === "true";
    if (config8.socksAuth === void 0) {
      config8.socksAuth = process.env.SOCKS_PROXY_USERNAME && process.env.SOCKS_PROXY_PASSWORD ? `${process.env.SOCKS_PROXY_USERNAME}:${process.env.SOCKS_PROXY_PASSWORD}` : "";
    }
    if (config8.siteConfig.registerReview === void 0)
      config8.siteConfig.registerReview = process.env.REGISTER_REVIEW === "true";
  }
  if (config8.auditConfig === void 0) {
    config8.auditConfig = new AuditConfig(
      process.env.AUDIT_ENABLED === "true",
      isTextAuditServiceProvider(process.env.AUDIT_PROVIDER) ? process.env.AUDIT_PROVIDER : "baidu",
      {
...
function getTextAuditServiceOptionFromString(value) {
  if (value === void 0)
    return 0 /* None */;
...
var apikeys = parseKeys(process.env.OPENAI_API_KEY);
var accessTokens = parseKeys(process.env.OPENAI_ACCESS_TOKEN);
var errorapikeys = {};
var availableKeys = {};
...
var nextKey = (() => {
  ...
  const getNextKey = () => {
    ...
    if (availableKeys.length) {
      ...
      return () => api.apiKey = next();
    } else {
      const next = loadBalancer(accessTokens);
      return () => api.accessToken = next();
    }
  };
  return () => {
...
var maxRetry = !isNaN(+process.env.MAX_RETRY) ? +process.env.MAX_RETRY : Math.max(apikeys.length, accessTokens.length);
var retryIntervalMs = !isNaN(+process.env.RETRY_INTERVAL_MS) ? +process.env.RETRY_INTERVAL_MS : 1e3;
async function initApi() {
  const config8 = await getCacheConfig();
  if (!config8.apiKey && !config8.accessToken)
    throw new Error("Missing OPENAI_API_KEY or OPENAI_ACCESS_TOKEN environment variable");
  if (isNotEmptyString(config8.apiKey)) {
    const OPENAI_API_BASE_URL = config8.apiBaseUrl;
    const OPENAI_API_MODEL = config8.apiModel;
    const model = isNotEmptyString(OPENAI_API_MODEL) ? OPENAI_API_MODEL : "gpt-3.5-turbo";
    const options = {
      apiKey: config8.apiKey,
      completionParams: { model },
      debug: !config8.apiDisableDebug
    };
    ...
    const model = isNotEmptyString(config8.apiModel) ? config8.apiModel : "gpt-3.5-turbo";
    const options = {
      accessToken: config8.accessToken,
      apiReverseProxyUrl: isNotEmptyString(config8.reverseProxy) ? config8.reverseProxy : "https://bypass.churchless.tech/api/conversation",
      model,
      debug: true
    };
    await setupProxy(options);
...
async function chatReplyProcess(options) {
  const config8 = await getCacheConfig();
  apikeys = parseKeys(config8.apiKey);
  const model = isNotEmptyString(config8.apiModel) ? config8.apiModel : "gpt-3.5-turbo";
  ...
  try {
    ...
    while (!response && retryCount++ < maxRetry) {
      index++;
      nextKey();
      console2.log("\u603B" + maxRetry + "\u5F00\u59CB\u5C1D\u8BD5" + index + api.apiKey);
      response = await api.sendMessage(message, options2).catch((error3) => {
        if (!error3.message.includes("please check your plan and billing details")) {
    ...
    console2.log("\u8FD4\u56DE\u6210\u529F\uFF0C\u672C\u6B21key" + index + api.apiKey);
    if (!response) {
      response = {
        data: null,
...
async function chatReplyProcess2(options) {
  ...
  try {
    ...
    while (!response && retryCount++ < maxRetry) {
      ...
      index++;
      console2.log("\u603B" + maxRetry + "\u5F00\u59CB\u5C1D\u8BD5" + index + apiKey);
      ...
      let imgRes = await (0, import_node_fetch2.default)(urlSubscription, {
        method: "POST",
        body: JSON.stringify(requestBody),
        headers
      }).catch((error3) => {
        if (error3.message.includes("Your prompt may contain text that is not allowed by our safety system")) {
          throw error3;
        }
        console2.log("\u672C\u6B21key error" + index + apiKey + "\u51FA\u9519");
        if (retryCount == maxRetry) {
          throw error3;
        }
      });
      ...
      if (!!imgData.error) {
        ...
        console2.log("\u672C\u6B21key" + index + apiKey + "\u51FA\u9519");
        if (retryCount == maxRetry) {
          throw imgData.error;
        }
        await sleep(retryIntervalMs);
    ...
    console2.log("\u753B\u56FE\u8FD4\u56DE\u6210\u529F\uFF0C\u672C\u6B21key" + index + apiKey);
    if (!response) {
      response = {
        data: null,
...
var cachedBanlance;
var cacheExpiration2 = 0;
async function fetchBalance() {
  const now = (/* @__PURE__ */ new Date()).getTime();
  if (cachedBanlance && cacheExpiration2 > now)
    return Promise.resolve(cachedBanlance.toFixed(3));
  const startDate = new Date(now - 90 * 24 * 60 * 60 * 1e3);
  const endDate = new Date(now + 24 * 60 * 60 * 1e3);
  const config8 = await getCacheConfig();
  ...
  let socksAgent;
  let httpsAgent;
  if (isNotEmptyString(config8.socksProxy)) {
    socksAgent = new import_socks_proxy_agent.SocksProxyAgent({
      hostname: config8.socksProxy.split(":")[0],
      port: parseInt(config8.socksProxy.split(":")[1]),
      userId: isNotEmptyString(config8.socksAuth) ? config8.socksAuth.split(":")[0] : void 0,
      password: isNotEmptyString(config8.socksAuth) ? config8.socksAuth.split(":")[1] : void 0
    });
  } else if (isNotEmptyString(config8.httpsProxy)) {
    httpsAgent = new HttpsProxyAgent(config8.httpsProxy);
  }
  try {
    let response = await (0, import_node_fetch2.default)(urlSubscription, { agent: socksAgent === void 0 ? httpsAgent : socksAgent, headers });
    if (!response.ok) {
      console2.error("\u60A8\u7684\u8D26\u6237\u5DF2\u88AB\u5C01\u7981\uFF0C\u8BF7\u767B\u5F55OpenAI\u8FDB\u884C\u67E5\u770B\u3002");
      return;
    }
    const subscriptionData = await response.json();
    const totalAmount = subscriptionData.hard_limit_usd;
    response = await (0, import_node_fetch2.default)(urlUsage, { agent: socksAgent === void 0 ? httpsAgent : socksAgent, headers });
    const usageData = await response.json();
    const totalUsage = usageData.total_usage / 100;
    cachedBanlance = totalAmount - totalUsage;
    cacheExpiration2 = now + 60 * 60 * 1e3;
    return Promise.resolve(cachedBanlance.toFixed(3));
  } catch (error3) {
...
async function chatConfig() {
  const config8 = await getOriginConfig();
  config8.balance = await fetchBalance();
  return sendResponse({
    type: "Success",
    data: config8
  });
...
async function setupProxy(options) {
  ...
  if (isNotEmptyString(config8.socksProxy)) {
    ...
    options.fetch = (url2, options2) => {
      return (0, import_node_fetch2.default)(url2, { agent, ...options2 });
    };
  } else {
    if (isNotEmptyString(config8.httpsProxy)) {
      const httpsProxy = config8.httpsProxy;
      if (httpsProxy) {
...
var auth = async (req, res, next) => {
  ...
  if (req.header("Authorization")) {
    try {
      ...
      const info = import_jsonwebtoken.default.verify(token, config8.siteConfig.loginSalt.trim());
      req.headers.userId = info.userId;
      const user = await getUserById(info.userId);
      if (user == null || user.status !== 0 /* Normal */)
        throw new Error("\u7528\u6237\u4E0D\u5B58\u5728 | User does not exist.");
      else
        next();
    } catch (error3) {
      console3.log(error3);
      res.send({ status: "Unauthorized", message: error3.message ?? "Please authenticate.", data: null });
    }
  } else {
    if (!config8.siteConfig.loginEnabled) {
      next();
    } else {
...
async function sendVerifyMail(toMail, verifyUrl) {
  const config8 = await getCacheConfig();
  ...
  mailHtml = mailHtml.replace(/\${SITE_TITLE}/g, config8.siteConfig.siteTitle);
  sendMail(toMail, `${config8.siteConfig.siteTitle} \u8D26\u53F7\u9A8C\u8BC1`, mailHtml, config8.mailConfig);
}
async function sendVerifyMailAdmin(toMail, verifyName, verifyUrl) {
  const config8 = await getCacheConfig();
  const templatesPath = path.join(__dirname, "templates");
  ...
  mailHtml = mailHtml.replace(/\${VERIFY_URL}/g, verifyUrl);
  mailHtml = mailHtml.replace(/\${SITE_TITLE}/g, config8.siteConfig.siteTitle);
  sendMail(toMail, `${config8.siteConfig.siteTitle} \u8D26\u53F7\u7533\u8BF7`, mailHtml, config8.mailConfig);
}
async function sendResetPasswordMail(toMail, verifyUrl) {
  const config8 = await getCacheConfig();
  const templatesPath = path.join(__dirname, "templates");
  ...
  mailHtml = mailHtml.replace(/\${VERIFY_URL}/g, verifyUrl);
  mailHtml = mailHtml.replace(/\${SITE_TITLE}/g, config8.siteConfig.siteTitle);
  sendMail(toMail, `${config8.siteConfig.siteTitle} \u5BC6\u7801\u91CD\u7F6E`, mailHtml, config8.mailConfig);
}
async function sendNoticeMail(toMail) {
  const config8 = await getCacheConfig();
  const templatesPath = path.join(__dirname, "templates");
  const mailTemplatePath = path.join(templatesPath, "mail.notice.template.html");
  let mailHtml = fs.readFileSync(mailTemplatePath, "utf8");
  mailHtml = mailHtml.replace(/\${SITE_DOMAIN}/g, config8.siteConfig.siteDomain);
  mailHtml = mailHtml.replace(/\${SITE_TITLE}/g, config8.siteConfig.siteTitle);
  sendMail(toMail, `${config8.siteConfig.siteTitle} \u8D26\u53F7\u5F00\u901A`, mailHtml, config8.mailConfig);
}
async function sendTestMail(toMail, config8) {
  return sendMail(toMail, "\u6D4B\u8BD5\u90AE\u4EF6|Test mail", "\u8FD9\u662F\u4E00\u5C01\u6D4B\u8BD5\u90AE\u4EF6|This is test mail", config8);
}
async function sendMail(toMail, subject, html, config8) {
  const mailOptions = {
    from: config8.smtpUserName,
    to: toMail,
    subject,
    html
  };
  const transporter = import_nodemailer.default.createTransport({
    host: config8.smtpHost,
    port: config8.smtpPort,
    secure: config8.smtpTsl,
    auth: {
      user: config8.smtpUserName,
      pass: config8.smtpPassword
    }
  });
...
async function getUserVerifyUrl(username) {
  const sign = getUserVerify(username);
  const config8 = await getCacheConfig();
  return `${config8.siteConfig.siteDomain}/#/chat/?verifytoken=${sign}`;
}
function getUserVerify(username) {
  return getVerify(username, "");
}
function getVerify(username, key) {
  const expired = (/* @__PURE__ */ new Date()).getTime() + 12 * 60 * 60 * 1e3;
...
async function getUserVerifyUrlAdmin(username) {
  const sign = getUserVerifyAdmin(username);
  const config8 = await getCacheConfig();
  return `${config8.siteConfig.siteDomain}/#/chat/?verifytokenadmin=${sign}`;
}
function getUserVerifyAdmin(username) {
  return getVerify(username, `|${process.env.ROOT_USER}`);
}
function checkUserVerifyAdmin(verify) {
  return checkVerify(verify);
}
async function getUserResetPasswordUrl(username) {
  const sign = getUserResetPassword(username);
  const config8 = await getCacheConfig();
  return `${config8.siteConfig.siteDomain}/#/chat/?verifyresetpassword=${sign}`;
}
function getUserResetPassword(username) {
  return getVerify(username, "|rp");
}
function checkUserResetPassword(verify, username) {
...
// src/middleware/rootAuth.ts
init_cjs_shims();
var import_jsonwebtoken2 = __toESM(require_jsonwebtoken());
var dotenv6 = __toESM(require("dotenv"));
dotenv6.config();
var rootAuth = async (req, res, next) => {
  const config8 = await getCacheConfig();
  if (config8.siteConfig.loginEnabled || true) {
    try {
      const token = req.header("Authorization").replace("Bearer ", "");
      const info = import_jsonwebtoken2.default.verify(token, config8.siteConfig.loginSalt.trim());
      req.headers.userId = info.userId;
      const user = await getUserById(info.userId);
      if (user == null || user.status !== 0 /* Normal */ || user.email.toLowerCase() !== process.env.ROOT_USER)
...
router.get("/chat-hisroty", auth, async (req, res) => {
  try {
    ...
    const result = [];
    chats.forEach((c) => {
      if (c.status !== 2 /* InversionDeleted */) {
        result.push({
          uuid: c.uuid,
...
router.post("/chat-delete", auth, async (req, res) => {
  try {
    const userId = req.headers.userId;
    const { roomId, uuid, inversion } = req.body;
    if (!roomId || !await existsChatRoom(userId, roomId)) {
      res.send({ status: "Fail", message: "Unknow room", data: null });
      return;
    }
...
router.post("/chat", auth, async (req, res) => {
  try {
    const { roomId, uuid, regenerate, prompt, options = {} } = req.body;
    const message = regenerate ? await getChat(roomId, uuid) : await insertChat(uuid, prompt, roomId, options);
...
router.post("/chat-process", [auth, limiter], async (req, res) => {
  ...
  let { roomId, uuid, regenerate, prompt, draw, options = {}, systemMessage, temperature, top_p } = req.body;
  const userId = req.headers.userId;
  ...
  try {
    const config8 = await getCacheConfig();
    if (config8.auditConfig.enabled || config8.auditConfig.customizeEnabled) {
      if (await containsSensitiveWords(config8.auditConfig, prompt)) {
        res.send({ status: "Fail", message: "\u542B\u6709\u654F\u611F\u8BCD | Contains sensitive words", data: null });
        return;
      }
    }
    message = regenerate ? await getChat(roomId, uuid) : await insertChat(uuid, prompt, roomId, options);
    let firstChunk = true;
...
${JSON.stringify(result.data)}`);
    ...
    res.write(JSON.stringify(error3));
  } finally {
    res.end();
    try {
      if (result == null || result === void 0 || result.status !== "Success")
...
router.post("/user-register", async (req, res) => {
  try {
    ...
    const config8 = await getCacheConfig();
    if (!config8.siteConfig.registerEnabled) {
    ...
    if (isNotEmptyString(config8.siteConfig.registerMails)) {
      let allowSuffix = false;
      const emailSuffixs = config8.siteConfig.registerMails.split(",");
    ...
    res.send({ status: "Fail", message: error3.message, data: null });
  }
});
...
router.post("/session", async (req, res) => {
  try {
    const config8 = await getCacheConfig();
    const hasAuth = config8.siteConfig.loginEnabled;
...
router.post("/user-login", async (req, res) => {
  try {
    ...
    }, config8.siteConfig.loginSalt.trim());
...
router.post("/user-info", auth, async (req, res) => {
  try {
    ...
    if (user == null || user.status !== 0 /* Normal */)
      throw new Error("\u7528\u6237\u4E0D\u5B58\u5728 | User does not exist.");
    await updateUserInfo(userId, { name, avatar, description });
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (config8.siteConfig.registerReview) {
      await verifyUser(username, 5 /* AdminVerify */);
      ...
      await verifyUser(username, 0 /* Normal */);
    }
...
router.post("/setting-base", rootAuth, async (req, res) => {
  try {
    const { apiKey, apiModel: apiModel2, apiBaseUrl, accessToken, timeoutMs, reverseProxy, socksProxy, socksAuth, httpsProxy } = req.body;
    if (apiKey == null && accessToken == null)
      throw new Error("Missing OPENAI_API_KEY or OPENAI_ACCESS_TOKEN environment variable.");
    const thisConfig = await getOriginConfig();
    thisConfig.apiKey = apiKey;
    thisConfig.apiModel = apiModel2;
...
router.post("/setting-site", rootAuth, async (req, res) => {
  try {
    const config8 = req.body;
    const thisConfig = await getOriginConfig();
    thisConfig.siteConfig = config8;
...
router.post("/mail-test", rootAuth, async (req, res) => {
  try {
    ...
    const userId = req.headers.userId;
    const user = await getUserById(userId);
    await sendTestMail(user.email, config8);
...
router.post("/setting-audit", rootAuth, async (req, res) => {
  try {
    const config8 = req.body;
    const thisConfig = await getOriginConfig();
    thisConfig.auditConfig = config8;
    const result = await updateConfig(thisConfig);
    clearConfigCache();
    if (config8.enabled)
      initAuditService(config8);
...
router.post("/audit-test", rootAuth, async (req, res) => {
  try {
    const { audit, text } = req.body;
    const config8 = await getCacheConfig();
    if (audit.enabled)
      initAuditService(audit);
    const result = await containsSensitiveWords(audit, text);
    if (audit.enabled)
      initAuditService(config8.auditConfig);
